...
DATASET_PATH = '/home/tenzing/Documents/project-rock/rknn_model_zoo/datasets/imagenet/calibration/dataset.txt'
...
labels = {0 : "flowers", 1 : "grass"}

# model = onnx.load(DEFAULT_ONNX_PATH)
...
if __name__ == '__main__':
    ...
    if platform.lower() in RKNPU1_TARGET:
        ret = rknn.load_onnx(model=model_path, inputs=['input.1'], input_size_list=[[3, 224, 224]])
    else:
        print("rk3568!")
        ret = rknn.load_onnx(model=model_path, inputs=['input.1'], input_size_list=[[1, 3, 224, 224]])
    ...
    # # Set inputs
    img = cv2.imread('/home/tenzing/Documents/project-rock/rknn_model_zoo/datasets/imagenet/calibration/image0.jpg')
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img = cv2.resize(img, (224, 224))
    # img = img.astype(np.float32) think rknn api does this for us
    ...
    # print the top-2 inferences class
